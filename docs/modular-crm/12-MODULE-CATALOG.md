...
| **Calendar** | Optional | Communication | on if available | yes | config `CALENDAR_PROVIDER_GOOGLE/MICROSOFT_ENABLED` (any)² | Activities | guarded (calendar module + settings + view) | done |
| **Automations** | Optional | Automation | off (SMB default) | yes | always³ | CRM records | guarded (workflow module + settings + routes/actions) | done |
| **AI Assistant** | Optional | AI | off | yes | config `AI_PROVIDERS` non-empty⁴ | — | guarded (AI chat page/side panel + tools + settings AI) | done |
...
- ² As shipped, Calendar's availability gate is the single deploy-config flag `IS_CALENDAR_MODULE_ENABLED` (`isEnvOnly`, default `true`), not the provider-specific flags listed above — see [IMPLEMENTATION-STATUS.md](IMPLEMENTATION-STATUS.md#pivot-deploy-config-availability-model-branch-featdeploy-config-module-provisioning) and [the deploy-config design spec](../superpowers/specs/2026-08-11-deploy-config-module-provisioning-design.md). The accepted shared-plumbing limitation (Accounts/OAuth/ChannelSync left reachable so Email is unaffected; `TimelineCalendarEventResolver` intentionally left ungated) is documented there.
- ³ As shipped, Automations' availability gate is the single deploy-config flag `IS_AUTOMATIONS_MODULE_ENABLED` (`isEnvOnly`, default `true`), object-backed via `effect.objectStandardIds: [workflow]` — the `workflow` object is hidden from object-nav when off (mirroring Dashboards), and `@RequireCapability` guards 21 `@Query`/`@Mutation` methods across five workflow resolvers plus the chat-invoked workflow AI tools (`WorkflowToolProvider`). See [IMPLEMENTATION-STATUS.md](IMPLEMENTATION-STATUS.md#pivot-deploy-config-availability-model-branch-featdeploy-config-module-provisioning) and [the deploy-config design spec](../superpowers/specs/2026-08-11-deploy-config-module-provisioning-design.md). The accepted limitation (the unauthenticated webhook trigger controller and background cron/async workflow runners left ungated, operator-controlled) is documented there.
- ⁴ As shipped, AI Assistant's availability gate is the single deploy-config flag `IS_AI_ASSISTANT_MODULE_ENABLED` (`isEnvOnly`, default `true`), not the `AI_PROVIDERS` config listed above — settings/nav-gated, not object-backed (`effect: {}`; agent/chat are core-schema entities, not object-metadata). `@RequireCapability` guards all 22 `@Query`/`@Mutation`/`@Subscription` methods across four AI resolvers (incl. the streaming subscription), and the Ask-AI panel open path is gated at its choke-point hook (`useOpenAskAiPageInSidePanel`). See [IMPLEMENTATION-STATUS.md](IMPLEMENTATION-STATUS.md#pivot-deploy-config-availability-model-branch-featdeploy-config-module-provisioning) and [the deploy-config design spec](../superpowers/specs/2026-08-11-deploy-config-module-provisioning-design.md). The accepted limitation (the shared `ai-generate-text` controller and the workflow `ai-agent` action left ungated) is documented there. This completes the modular-CRM deploy-config rollout — all five optional modules (Dashboards, Email, Calendar, Automations, AI Assistant) are now deploy-config gated.
