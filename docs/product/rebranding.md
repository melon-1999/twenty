...
- **`twenty-icons.com`** (Favicon-Service für Firmenlogos): funktionaler Dienst, per `ALLOW_REQUESTS_TO_TWENTY_ICONS=false` abschaltbar; geprüft in `search.service.ts`, `navigation-menu-item-record-identifier.service.ts`, `client-config.service.ts` **und jetzt auch** `SignInUpService.uploadInferredWorkspaceLogo` (`sign-in-up.service.ts`). **Behoben (vormals Datenschutz-Befund, live beobachtet):** Bei Workspace-Erstellung baute dieser Pfad die `TWENTY_ICONS_BASE_URL`-URL (`packages/twenty-shared/src/constants/TwentyIconsBaseUrl.ts`) inline und lud sie ungefragt hoch, ohne das Flag zu konsultieren — z. B. Fetch von `twenty-icons.com/testfirma.de` beobachtet. Der Aufruf prüft das Flag nun vorab; bei `false` bleibt das Workspace-Logo einfach unset (Frontend fällt bereits auf den Initialen-Avatar zurück). (`getLogoUrlFromDomainName` bleibt ein separates, rein Front-/Website-seitiges Util ohne Bezug zu diesem Server-Pfad.) **Deployment-Hinweis:** Auf Novi-Kundeninstanzen `ALLOW_REQUESTS_TO_TWENTY_ICONS=false` per Config setzen (Upstream-Default des Flags bleibt unverändert `true`).
...
- **`IS_MULTIWORKSPACE_ENABLED`** trägt jetzt fork-eigenes `isEnvOnly: true` in `config-variables.ts` (ebenfalls Datei mit hoher Upstream-Churn) — bei Rebase beachten.
...
- Enforcement: Vite-Plugin `packages/twenty-front/src/config/assertProductionBrandingPlugin.ts`, eingehängt in `vite.config.ts` (2-Zeilen-Diff). Blockt `vite build` im Mode `production` und prüft zusätzlich `index.html`, `public/manifest.json` sowie `DOCUMENTATION_BASE_URL` (Platzhalter `docs.example.com`); der Release-Workflow prüft dieselben Werte serverseitig.
...
- **Community-Seite entfernt**: `/settings/community` leitet auf den neuen Legal-Bereich um; Nav-Item ersetzt. `SettingsCommunity.tsx` (GitHub-Icon, Source-/Changelog-Links) war danach komplett unreferenziert und wurde gelöscht. Labs (`SettingsLabContent`) war nur dort eingebunden und ist damit unerreichbar; `PUBLIC_FEATURE_FLAGS` serverseitig unangetastet.
...
- **Navigation "Documentation"** (docs.twenty.com) entfernt. `DOCUMENTATION_BASE_URL` (twenty-shared) zeigt jetzt auf den Platzhalter `docs.example.com`; alle kontextuellen "Learn more"-Doku-Links (Workflow-Form-Builder, AI-Setup, Billing-Credits, Apps-Developer-Tab, "My first dashboard"-Note, OpenAPI/MCP-Doku-Links) laufen darüber. Eigene Doku-Inhalte sind Open Item.
...
- **Bewusst tote Dateien** (kein Route-Konsument mehr, für minimalen Upstream-Diff behalten): `SettingsLegalDpa.tsx`, `SettingsLegalDpaNew.tsx`, `OnboardingTrustBadges.tsx`, `SettingsLabContent.tsx` (einziger Konsument war die gelöschte Community-Seite).
- **Standard-Prefill neutralisiert**: `prefill-people.util.ts` lud Beispiel-Avatare von `twentyhq.github.io` (Outbound-Request an Upstream-Domain in jedem echten Kunden-Workspace) — jetzt `avatarUrl: ''` (Initialen-Avatare); `prefill-workflows.util.ts` zeigte `twenty.com` als Beispielwert im Workflow-Variablen-Picker — jetzt `example.com`.
- **Seed-Daten-Leak "Star History"**: Der Dev-/Demo-Seeder legte ein Navigation-Item "Star History" (IconStar) samt Standalone-Page mit IFrame-Widget "Twenty Star History" (`star-history.com/?repos=twentyhq/twenty`) direkt in der Kunden-Navigation an — als DB-Daten unsichtbar für statische Grep-Scans. Seeds jetzt neutral ("Documentation", IconBook2, `docs.example.com`); bestehende Dev-Datenbanken brauchen ein Re-Seed (`database:reset`) oder ein manuelles Update der Zeilen in `core."navigationMenuItem"`/`core."pageLayoutWidget"` (gilt auch für alte `commandMenuItem`-Zeilen "Go to Community Settings" aus Seeds vor dem Legal-Umbau).
...
- **Regressionstest**: `packages/twenty-shared/src/utils/branding/__tests__/customerVisibleBrandingLeaks.test.ts` scannt eine feste Liste kundensichtbarer Quelldateien (inkl. Dev-Seeder-Navigation/-Widgets, Standard-App-Dashboard-Widgets, `DocumentationBaseUrl.ts`) auf `twentyhq`, `twenty.com`, `github`, `discord`, `star-history`, ChatGPT-App-Links und "Powered by" — bewusst NICHT global auf "Twenty" (interne Namen und Lizenztexte bleiben legitim). Zusätzlich prüft `seed-customer-visible-branding.util.spec.ts` (twenty-server) die tatsächlichen Seed-Funktionsausgaben.
...
## Single-Company-Modus (eine Instanz = ein Unternehmen)

Hosting-Modell: eine Instanz pro Kunde (eigene Subdomain), genau ein Workspace, mehrere Benutzer. Technische Basis ist `IS_MULTIWORKSPACE_ENABLED=false` (Code-Default; produktiv explizit setzen).

- **Backend-Guard ist Upstream-Code, kein eigener**: Der einzige produktive Workspace-Erstellungspfad ist `SignInUpService.signUpOnNewWorkspace`; dessen `assertWorkspaceCreationAllowed` → `assertSignUpEnabled` erlaubt Workspace-Erstellung nur bei `IS_MULTIWORKSPACE_ENABLED=true` oder `workspaceCount === 0`. Ergebnis: 0 Workspaces → erster Benutzer richtet das Unternehmen ein; 1 Workspace → jede weitere Erstellung (auch `signUpInNewWorkspace` per GraphQL, auch durch Server-Admins) wirft `SIGNUP_DISABLED`. SSO/OAuth-Provisioning, Einladungen und Admin-Panel erzeugen nie Workspace-Zeilen. Verhalten festgenagelt durch `sign-in-up.service.spec.ts` (neu) — schlägt fehl, wenn ein Upstream-Merge die Guard-Reihenfolge oder -Semantik ändert.
- **Public-Invite-Link jetzt bei Erstellung deaktiviert (Hardening)**: Der Entity-Default `isPublicInviteLinkEnabled=true` war auf Single-Instance-Deployments ein Leck — der `inviteHash` steckt unrotiert in jeder Invite-Mail-URL, wodurch sich beliebige E-Mail-Adressen per `signUpInWorkspace(workspaceInviteHash)` selbst einschreiben konnten, `SIGNUP_DISABLED` umgehend. `SignInUpService.signUpOnNewWorkspace` setzt beim Anlegen jetzt `isPublicInviteLinkEnabled: IS_MULTIWORKSPACE_ENABLED` (Single-Instance → `false` bei Geburt; Twenty-Cloud/Multi-Workspace behält `true`). Bei `IS_MULTIWORKSPACE_ENABLED=false` lehnt `checkAccessForSignIn` (`auth.service.ts`) den Public-Invite-Signup zur Laufzeit unabhängig vom DB-Wert ab — der Toggle existiert im UI ohnehin nur im Multi-Workspace-Betrieb (siehe unten). Das SQL-`UPDATE` auf bestehende Pre-Fix-Workspaces bleibt reine Hygiene, kein Sicherheitserfordernis mehr: `UPDATE core.workspace SET "isPublicInviteLinkEnabled" = false WHERE id = '<workspaceId>'`. Getestet in `sign-in-up.service.spec.ts`.
- **Frontend**: das Gate auf der "Create a workspace"-Kachel in `SignInUpGlobalScopeForm.tsx` ist sichtbar nur bei `isMultiWorkspaceEnabled` ODER wenn kein Workspace verfügbar ist (First-Setup — auf einer frischen Instanz ist dieser Render-Pfad der echte Ersteinrichtungs-Screen, und der Zurück-Button der Erstellungs-Form landet dort). "Create Workspace" im Workspace-Dropdown war upstream bereits geflaggt, Switcher-Liste erscheint nur bei >1 verfügbarem Workspace. Zusätzlich bei `!isMultiWorkspaceEnabled` ausgeblendet, da sie im Single-Company-Modus auf tote Server-Pfade zeigen würden: der "Invite by Link"-Toggle in Settings → Security (`SettingsSecurityAuthProvidersOptionsList.tsx`) sowie in Settings → Mitglieder → Einladen (`SettingsWorkspaceMembersInviteTab.tsx`) die Invite-Link-Section und die Genehmigte-Domänen-Section. Alles durch Jest-Tests gelockt (`SignInUpGlobalScopeForm.test.tsx` inkl. Leer-Zustand, `MultiWorkspaceDropdownDefaultComponents.test.tsx`, sowie neue Tests für die beiden Settings-Komponenten).
- **Wording "Unternehmen" statt "Arbeitsbereich"**: nur in den de-DE-Katalogen (msgstr), englische msgids bleiben upstream-identisch — kein msgid-Bruch, keine Komponenten-Änderung. 14 Front-Einträge (`de-DE.po` + kompiliertes `generated/de-DE.ts`) und 1 Server-Eintrag ("Workspace name is required" → "Unternehmensname ist erforderlich"). Gegen Crowdin-Revert gesichert durch `twenty-front/src/locales/__tests__/germanCompanyWording.test.ts` bzw. `twenty-server/.../i18n/locales/__tests__/german-company-wording.spec.ts` (Server-Eintrag serverseitig getestet, damit nx-affected greift). Englische UI sagt weiterhin "workspace" (bewusst). **Bewusste Kollision**: "Unternehmen" ist zugleich das deutsche Label des Company-Objekts im CRM; die betroffenen Strings erscheinen aber nur in Onboarding/Auth (vor dem CRM) — akzeptiert, bei Verwirrung Alternative "Firmenkonto" erwägen.
- **Weitere Benutzer** treten per Einladung bei (Settings → Members, `signUpInWorkspace`-Flow) oder über Approved Access Domains (letztere nur im Multi-Workspace-Betrieb). **Runbook-Hinweis:** Im Single-Company-Modus (`IS_MULTIWORKSPACE_ENABLED=false`) ist der `approvedAccessDomains`-Early-Return in `checkAccessForSignIn` (`auth.service.ts`) abgeschaltet — eine validierte Domain erlaubt keinen Self-Join mehr, unabhängig von `IS_EMAIL_VERIFICATION_REQUIRED` und `isPublicInviteLinkEnabled`. Beitritt dann ausschließlich per persönlicher Einladung. Domain-CRUD und Workspace-Discovery bleiben serverseitig unverändert; im UI ist die Approved-Domains-Section (Settings → Mitglieder → Einladen) im Single-Company-Modus ausgeblendet (siehe Frontend-Punkt oben).
- **`IS_MULTIWORKSPACE_ENABLED` ist jetzt env-only**: `isEnvOnly: true` in der Config-Metadata verhindert, dass ein Server-Admin das Single-Company-Lockdown-Flag zur Laufzeit übers Admin-Panel umschaltet — das hätte Public-Invite-Link-, Approved-Domain- und Workspace-Creation-Guard gleichzeitig ausgehebelt.
- **Sign-up-Picker blendet Approved-Domain-Workspaces bei Flag=false aus**: `findAvailableWorkspacesByEmail` (`user-workspace.service.ts`) füllt `availableWorkspacesForSignUp` aus Approved Access Domains nur noch bei `IS_MULTIWORKSPACE_ENABLED=true`, da der Server den Self-Join sonst ohnehin ablehnt (siehe Punkt 17) und die Kachel samt `inviteHash` sonst ein Fail-closed-Dead-End wäre.
...
15. ERLEDIGT (Upstream-Bug, minimal im Fork gepatcht): Settings-Sidebar auf Mobile (<768px) war bei Direktaufruf/Reload von `/settings/*` nicht bedienbar (Drawer collapsed, keine Öffnen-Affordance) und nach Auswahl einer Sektion gab es keinen Weg zurück zur Sektionsliste. Fix: `PageCardHeader.tsx` rendert den vorhandenen Expand-Button (`NavigationDrawerCollapseButton`) jetzt auch auf Mobile, sobald der Drawer collapsed ist; der Button unterdrückt sich auf Mobile außerhalb des Settings-Drawers selbst (bestehender Guard in `NavigationDrawerCollapseButton.tsx`). 3-Zeilen-Diff statt Two-Pane-Refactor, upstream-tauglich; Desktop-Verhalten unverändert (Settings-Drawer dort immer expanded).
...
18. Zwei client-seitige `twenty-icons`-Callsites bleiben ungegated: `BackgroundMockTableRow.tsx` (404-Seite) und `OnboardingImportPreviewCompanies.tsx` — beide Browser-seitig, Upstream-Code, niedrige Priorität.
