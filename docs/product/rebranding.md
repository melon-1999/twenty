...
- **`twenty-icons.com`** (Favicon-Service für Firmenlogos): funktionaler Dienst, per `ALLOW_REQUESTS_TO_TWENTY_ICONS=false` abschaltbar — deckt aber nicht alle Aufrufer ab; das Flag wird nur in `search.service.ts`, `navigation-menu-item-record-identifier.service.ts` und `client-config.service.ts` geprüft. **Datenschutz-Befund (live beobachtet):** Bei Workspace-Erstellung baut `SignInUpService.uploadInferredWorkspaceLogo` (`sign-in-up.service.ts`) die `TWENTY_ICONS_BASE_URL`-URL (`packages/twenty-shared/src/constants/TwentyIconsBaseUrl.ts`) inline und lädt sie ungefragt hoch — z. B. Fetch von `twenty-icons.com/testfirma.de` beobachtet; dieser Pfad konsultiert das Flag nicht, der Leak besteht also unabhängig davon. (`getLogoUrlFromDomainName` ist ein separates, rein Front-/Website-seitiges Util ohne Bezug zu diesem Server-Pfad.) Entscheidung offen: eigener Icon-Service, abschalten oder akzeptieren.
...
- **Public-Invite-Link jetzt bei Erstellung deaktiviert (Hardening)**: Der Entity-Default `isPublicInviteLinkEnabled=true` war auf Single-Instance-Deployments ein Leck — der `inviteHash` steckt unrotiert in jeder Invite-Mail-URL, wodurch sich beliebige E-Mail-Adressen per `signUpInWorkspace(workspaceInviteHash)` selbst einschreiben konnten, `SIGNUP_DISABLED` umgehend. `SignInUpService.signUpOnNewWorkspace` setzt beim Anlegen jetzt `isPublicInviteLinkEnabled: IS_MULTIWORKSPACE_ENABLED` (Single-Instance → `false` bei Geburt; Twenty-Cloud/Multi-Workspace behält `true`). Admin kann pro Workspace weiterhin über Settings → Security umschalten. Bestehende (vor diesem Fix angelegte) Workspaces sind nicht rückwirkend betroffen — dort Toggle in Settings → Security setzen oder einmalig `UPDATE core.workspace SET "isPublicInviteLinkEnabled" = false WHERE id = '<workspaceId>'`. Getestet in `sign-in-up.service.spec.ts`.
- **Frontend**: einziger Produktions-Change ist das Gate auf der "Create a workspace"-Kachel in `SignInUpGlobalScopeForm.tsx`: sichtbar nur bei `isMultiWorkspaceEnabled` ODER wenn kein Workspace verfügbar ist (First-Setup — auf einer frischen Instanz ist dieser Render-Pfad der echte Ersteinrichtungs-Screen, und der Zurück-Button der Erstellungs-Form landet dort). "Create Workspace" im Workspace-Dropdown war upstream bereits geflaggt, Switcher-Liste erscheint nur bei >1 verfügbarem Workspace. Alles durch Jest-Tests gelockt (`SignInUpGlobalScopeForm.test.tsx` inkl. Leer-Zustand, `MultiWorkspaceDropdownDefaultComponents.test.tsx`).
...
- **Weitere Benutzer** treten per Einladung bei (Settings → Members, `signUpInWorkspace`-Flow) oder über Approved Access Domains; beides unverändert Upstream. **Runbook-Hinweis:** Wird `IS_EMAIL_VERIFICATION_REQUIRED` später aktiviert, greift in `checkAccessForSignIn` (`auth.service.ts`) der `approvedAccessDomains`-Early-Return vor dem Public-Invite-Link-Check — eine validierte Domain erlaubt Self-Join unabhängig von `isPublicInviteLinkEnabled`.
...
16. Defense-in-Depth für Public-Invite-Link-Härtung (siehe Single-Company-Modus oben): aktueller Fix ist ein Birth-Time-Snapshot (`isPublicInviteLinkEnabled` wird nur bei Workspace-Erstellung aus `IS_MULTIWORKSPACE_ENABLED` gesetzt, DB-Spalte danach unabhängig). Ein optionaler Runtime-Check in `checkAccessForSignIn` (`auth.service.ts`, Public-Link-Zweig), der Public-Link-Signup bei `IS_MULTIWORKSPACE_ENABLED=false` generell ablehnt, würde die Invariante config-getrieben statt DB-getrieben machen — schützt dann auch Pre-Fix-Workspaces und den Fall, dass ein Admin `isPublicInviteLinkEnabled` nachträglich wieder aktiviert. Bewusst nicht umgesetzt (minimaler Diff); bei Bedarf nachrüsten.
