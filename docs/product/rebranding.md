...
15. Upstream-Bug (nicht gepatcht, minimaler Diff): Settings-Sidebar auf Mobile (<768px) bei Direktaufruf/Reload von `/settings/*` nicht bedienbar (Drawer collapsed, keine Öffnen-Affordance; nur In-App-Navigation innerhalb von Settings expandiert ihn). Betroffene Dateien (`AppNavigationDrawer.tsx`, `NavigationDrawer.tsx`, `useNavigationDrawerExpanded.ts`, `useOpenSettings.ts`) byte-identisch mit upstream main — kein Fork-Fehler; Mobile-Hauptnavigation hat ohnehin keinen Settings-Einstieg. Bewertung: upstream MAJOR, Fix (Two-Pane-Mobile-Pattern) gehört upstream
